...
2. **หน้า S5** ต้องแสดง chip ยืนยันว่าเบลอใบหน้าเรียบร้อยก่อนอัปโหลดเสมอ (PDPA transparency)
   — ⚠️ **ยังไม่ทำ (2026-08-28)** `lib/capture.ts` บันทึก `faceBlur.method: "NOT_WIRED"`
   ไม่มี detector ที่รันในเบราว์เซอร์ ข้อกำหนดนี้**ไม่ถูกลบ** แต่เลื่อนออกไป และหน้ากล้อง
   บอกผู้ถ่ายตรง ๆ ว่าต้องเลี่ยงไม่ให้คนติดเฟรมเอง — ห้ามมีจอไหนอ้างว่าเบลอให้แล้ว
...
## 1.6 "OSA %" บนหน้าจอ — ห้าตัวเลข ไม่ใช่ตัวเดียว

> หัวข้อนี้ว่าด้วย **ตัวเลขไหนคือช่วงเวลาไหน** ส่วนวิธีคำนวณตัวเลขเองอยู่ที่
> [osa.md](osa.md)

ทุกจอเขียนว่า "OSA %" เหมือนกัน แต่เบื้องหลังเป็นคนละค่า และ**ตั้งใจให้เป็นคนละค่า**:

| ที่แสดง | คืออะไร | ต้นทาง |
| :-- | :-- | :-- |
| หน้าผลวิเคราะห์ (S7) | OSA ของภาพใบเดียวที่เพิ่งถ่าย | `analysis.osa_score` |
| เช็คเอาต์/เปรียบเทียบ "ก่อน" | ภาพ BEFORE **ใบแรก**ของการเข้าร้านครั้งนี้ | `visit.osa_before` |
| เช็คเอาต์ "หลัง" | ภาพ AFTER **ใบล่าสุด**ของการเข้าร้านครั้งนี้ | `visit.osa_after` |
| การ์ดหน้าเส้นทาง / dashboard | **แถววิเคราะห์ล่าสุดของร้าน** — ชั้นไหนก็ได้ เฟสไหนก็ได้ ครั้งไหนก็ได้ | `store_risk.last_osa` |
| กราฟแนวโน้ม (W1) | ค่าเฉลี่ยรายสัปดาห์ | `GET /v1/analytics/osa` |

ข้อบังคับที่ตามมา:

1. **ตัวเลขบนการ์ดต้องมีบรรทัดบอกที่มาเสมอ** (`<OsaSource>`) — ก่อน/หลังเติมของ · ชั้นวาง
   (ถ้าจอนั้นรู้ชื่อ) · "วัดเมื่อ N วันก่อน" เมื่อค่านั้นเก่ากว่าการเข้าร้านครั้งล่าสุด
   ถ้าไม่มีบรรทัดนี้ ผู้ใช้จะอ่านค่า "ก่อนเติมของ" เป็นผลงานของตัวเอง
2. **การติ๊กงานว่าเติมแล้วไม่ทำให้ OSA ขยับ** — ขยับได้ทางเดียวคือมีภาพใหม่ถูกวิเคราะห์
   ห้ามจอไหนประมาณค่าหลังเติมของจากจำนวนงานที่ติ๊ก
3. **เกณฑ์ OK / LOW / CRITICAL อยู่ที่ `backend/app/core/config.py` ที่เดียว**
   ฝั่งเว็บ mirror ไว้ที่ `frontend/src/lib/osa.ts` และ
   `backend/tests/unit/test_osa_thresholds_contract.py` จะแดงถ้าสองฝั่งไม่ตรงกัน
   — เคยเพี้ยนมาแล้ว (เว็บใช้ 70 ขณะที่ backend ใช้ 75) ทำให้ชั้นวางเดียวกันขึ้น
   CRITICAL หน้าหนึ่งและ LOW อีกหน้าหนึ่ง

---

## 1.7 กรอบตรวจจับบนภาพผล — "ต้องตรวจสอบ" หมายถึงอะไร

> หัวข้อนี้ว่าด้วย **สิ่งที่วาดลงบนภาพ** ส่วนเกณฑ์คะแนนที่ทำให้กล่องหนึ่งถูกเรียกว่า
> ไม่มั่นใจอยู่ที่ [osa.md](osa.md) ขั้น 1

`is_low_confidence` (คะแนน 0.35–0.55) ติดกับ detection **ทุกชนิด** ทั้งสินค้า
ช่องว่าง และป้ายราคา แต่หน้าจอสองแบบต้องการคนละอย่าง จึงแยกออกจากกันชัดเจน:

| สิ่งที่วาด | เกณฑ์ | เหตุผล |
| :-- | :-- | :-- |
| **เส้นประ** รอบกรอบ | ทุกกล่องที่ < 0.55 | เป็นการบอกสถานะเฉย ๆ ไม่กินที่ ไม่ทับกัน |
| **ป้าย "ต้องตรวจสอบ"** | เฉพาะกล่อง `GAP` ที่ < 0.55 | ป้ายนี้คือ**คำสั่งให้ไปตัดสินใจ** สินค้าที่โมเดลไม่มั่นใจไม่มีอะไรให้พนักงานตัดสินใจ |

ภาพตู้จริงใบหนึ่งมี detection ~69 กล่อง ในนั้นราว 3.7 กล่องคะแนนต่ำกว่า 0.55
แต่เป็นช่องว่างจริงแค่ 0.5 กล่อง — ถ้าติดป้ายทุกกล่อง **ป้ายราว 87% จะเป็นของ
ที่พนักงานทำอะไรกับมันไม่ได้** และป้ายมีความกว้างคงที่ ไม่มีการหลบกัน จึงทับกันเอง
บนชั้นวางที่ของแน่น

### ฟิลเตอร์สองตัวที่ชื่อคล้ายกันแต่คนละความหมาย

| ฟิลเตอร์ | ใช้ที่ | แสดง |
| :-- | :-- | :-- |
| `LOW_CONF_GAP` — ชิป "ต้องตรวจสอบ" | มือถือ (S8) | เฉพาะ `GAP` ที่ไม่มั่นใจ = ตรงกับตัวเลขบนไทล์ ซึ่งนับจาก `findings` |
| `LOW_CONF` — ชิป "ความมั่นใจต่ำ" | เว็บ (Evidence Viewer) | ทุกกล่องที่ไม่มั่นใจ เพราะผู้จัดการกำลังตรวจสอบ**โมเดล** ไม่ใช่กำลังจะไปเติมของ |

ทั้งสองอยู่ในคอมโพเนนต์เดียวกันคือ
[`DetectionOverlay.tsx`](../frontend/src/components/shelf/DetectionOverlay.tsx)
ถ้ารวมสองอันนี้เป็นตัวเดียวเมื่อไหร่ จอใดจอหนึ่งจะผิดเสมอ

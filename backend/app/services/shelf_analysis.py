...
def analyse(
    ...
    *,
    ...
    rows = _cluster_into_rows(kept, config)
    ...
    if shelf_area <= 0:
        # Price tags only: we can see the shelf edge but nothing about what is
        # or isn't on it. Reporting either 0% or 100% here would be a fabrication.
        raise NoShelfDetectedError(
...
def _cluster_into_rows(
    detections: Sequence[Detection], config: AnalysisConfig
) -> list[list[Detection]]:
    """Group detections into shelf rows by the y-centre of each box.
...
def _measure_rows(
    ...
    for index, row in enumerate(rows, start=1):
        ...
        # A row of nothing but price tags has no shelf area to judge, so it
        # contributes nothing rather than a misleading zero-gap reading.
        metrics.append(
            RowMetrics(
                ...
                detection_count=sum(1 for d in row if d.semantic_type in _AREA_TYPES),
            )
        )
...
def _build_findings(
    ...
    findings = [
        ...
        for d in detections
        if d.semantic_type is SemanticType.GAP
    ]
    findings.sort(key=lambda f: (f.shelf_row_index, f.position_label))

...
- [x] **Task 8: ป้ายกำกับที่มาของ OSA บน 4 จอ**
  ...
  - Acceptance:
    - [x] `Store` (types) และ `toStore` รับสามฟิลด์ใหม่ผ่าน `lib/api/routes.ts`
    - [x] มี helper เดียว (ใน `lib/osa.ts`) ที่แปลง phase/category/at → ป้ายภาษาไทย
    - [x] ป้ายบอกเฟสได้ถูกต้อง: BEFORE → สื่อว่าเป็นค่าก่อนเติมของ · AFTER → หลังเติมของ
    - [x] ไม่มีหน้าไหนเรียกเลขของการตรวจครั้งล่าสุดว่า "ครั้งก่อน" อีก
    - [x] ชื่อหมวดชั้นวางแสดงเมื่อร้านนั้นมีมากกว่าหนึ่งหมวด (ตาม Open Question 1)
    - [x] ร้านที่ไม่เคยตรวจยังใช้ข้อความเดิม "ยังไม่เคยตรวจชั้นวางที่ร้านนี้"
  - Verify:
    - [x] `grep -rn "OSA ครั้งก่อน" frontend/src` → ไม่พบ
    - [x] `npx tsc --noEmit` สะอาด · `npm run lint` ≤ 18
    - [x] `npm run test:e2e` + `npm run test:e2e:demo` ผ่านทั้งคู่
    - [x] manual: การ์ดร้านที่เพิ่งตรวจเสร็จโดยไม่ถ่าย AFTER อ่านแล้วเข้าใจได้ว่าเลขนี้
...
- [x] **Task 9: บันทึกนิยาม OSA ทั้ง 5 ตัวไว้ให้คนอ่านได้**

  ทั้งทีมและ tester ต้องอ่านได้จากที่เดียวว่า OSA แต่ละที่คือเลขอะไร ไม่งั้นจะมีรายงาน
  "ตัวเลขไม่ตรง" ซ้ำเดิมทุกรอบ

  - Acceptance:
    - [x] [docs/ui.md](../docs/ui.md) มีหัวข้อ OSA ที่ระบุครบทั้ง 5 นิยาม พร้อมชี้ไฟล์ต้นทาง
    - [x] [docs/uat.md](../docs/uat.md) เขียนให้ tester ว่า **ติ๊ก task ว่าเติมแล้วไม่ทำให้
          OSA ขยับ** ต้องถ่ายภาพหลังเติมของเท่านั้น
    - [x] บันทึกว่าเกณฑ์สถานะคือ 75/90 และเคยเพี้ยนเป็น 70 ที่ frontend มาก่อน
  - Verify:
    - [x] อ่านทวนแล้วตรงกับพฤติกรรมหลัง Task 1–8 จริง ไม่ใช่ตรงกับแผน

...
### - [x] T1: Repo skeleton + Docker Compose on non-conflicting ports
...
### - [x] T2: Contracts package — OpenAPI 3.1 + Pydantic models
...
### - [x] T3: ML scaffold, dataset extraction, class map
...
### - [x] T4: Training pipeline + smoke run (measure epoch time)
...
### - [x] T5: ⏳ Launch full training in background
...
### - [x] T6: Backend scaffold — config, logging, health
...
### - [x] T9: ⭐ OSA rules engine (pure) + 6 fixtures
...
### - [x] T10: MockMLClient + MinIO storage adapter
...
### - [x] T11: Route + visit check-in slice
...
### - [x] T12: ⭐ Capture → job → result (the golden path)
...
### - [x] T13: Verify → task → checkout slice
...
### - [x] T14: Manager analytics (read-only)
...
### - [x] T15: Inference service over trained ONNX
...
### - [x] T16: 🔀 The flip — `ML_CLIENT=mock` → `http`
...
### - [x] T17: Golden-path integration test + full suite
...
### - [x] T18: Review, fix, ship checklist
